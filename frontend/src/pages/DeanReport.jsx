...
function DeanReport() {
  ...
  useEffect(() => {
    const fetchReport = async () => {
      ...
      setError("");
      try {
        const { data } = await api.get("dean-report/");
        setReport(data);
        setActiveYear((data.years || []).length > 0 ? "All Time" : "");
      } catch (err) {
        console.error("Error loading Dean report:", err);
        setError(
          err?.response?.data?.detail ||
            "Unable to load Dean report. Check your access and try again.",
        );
      } finally {
        setLoading(false);
      }
  ...
  const departmentSummaryForPeriod = useMemo(() => {
      ...
      .map((department) => ({
        ...
        period_metrics:
          activeYear === "All Time"
            ...
            : department.per_year?.[activeYear] || {
                publications: 0,
                conferences_attended: 0,
                grants: 0,
                patents: 0,
  ...
  }, [report.departments, activeYear]);

  const activeDepartmentsForPeriod = departmentSummaryForPeriod.filter(
    ...
    <div className="min-h-screen bg-slate-50 flex flex-col">
      ...
      <main className="flex-1 w-full max-w-7xl mx-auto py-8 px-4 sm:px-6 lg:px-8">
        <div className="space-y-6">
          ...
          {loading && <p className="text-sm text-slate-500">Loading Dean report...</p>}
          ...
          {!loading && !error && (
            <>
              <section className="grid gap-4 md:grid-cols-2 xl:grid-cols-5">
                <div className="rounded-2xl border border-slate-200 bg-white p-5">
                  ...
                  <p className="mt-3 text-3xl font-semibold text-slate-900">
                    {report.summary?.department_count || 0}
                  </p>
                </div>
                <div className="rounded-2xl border border-slate-200 bg-white p-5">
                  <p className="text-xs uppercase tracking-[0.2em] text-slate-400">
                ...
                <div className="rounded-2xl border border-slate-200 bg-white p-5">
                  <p className="text-xs uppercase tracking-[0.2em] text-slate-400">
                    Active Departments ({activeYear})
                  </p>
                  <p className="mt-3 text-3xl font-semibold text-slate-900">
                    {activeDepartmentsForPeriod}
                  </p>
                </div>
                <div className="rounded-2xl border border-slate-200 bg-white p-5">
                  <p className="text-xs uppercase tracking-[0.2em] text-slate-400">
              ...
              <section className="rounded-2xl border border-slate-200 bg-white p-6">
                <div className="flex items-center justify-between">
                  <h4 className="text-sm font-semibold uppercase tracking-[0.2em] text-slate-400">
                    Department Productivity ({activeYear})
                  </h4>
                  <span className="text-xs text-slate-500">
                    Grouped by faculty departments
                  </span>
                </div>
                <div className="mt-5 overflow-hidden rounded-2xl border border-slate-200">
                  <table className="w-full text-left text-sm">
                    ...
                    <tbody>
                      {departmentSummaryForPeriod.length === 0 ? (
                        <tr>
                        ...
                        departmentSummaryForPeriod.map((department, index) => (
                          <tr key={department.code} className="border-t border-slate-200">
                            <td className="px-4 py-3 text-slate-600">{index + 1}</td>
                            <td className="px-4 py-3 text-slate-900">{department.name}</td>
                            <td className="px-4 py-3 text-slate-600">
                              {department.staff_count || 0}
                            </td>
                            <td className="px-4 py-3 text-slate-900 font-semibold">
                              {department.period_metrics.total || 0}
                            </td>
                            <td className="px-4 py-3 text-slate-600">
                              {department.period_metrics.publications || 0}
                            </td>
                            <td className="px-4 py-3 text-slate-600">
                              {department.period_metrics.conferences_attended || 0}
                            </td>
                            <td className="px-4 py-3 text-slate-600">
                              {department.period_metrics.grants || 0}
                            </td>
                            <td className="px-4 py-3 text-slate-600">
                              {department.period_metrics.patents || 0}
                            </td>
                            <td className="px-4 py-3 text-slate-600">
                              {department.period_metrics.innovations || 0}
                            </td>
                            <td className="px-4 py-3 text-slate-600">
                              {department.last_publication_year || "—"}
                            </td>
                          </tr>
                        ))
                      )}
                        ...
                        report.recent_publications.map((item, index) => (
                          <tr key={item.id} className="border-t border-slate-200">
                            ...
                            <td className="px-4 py-3 text-slate-900">
                              {item.user__department__name || item.user__department || "—"}
                            </td>
                            <td className="px-4 py-3 text-slate-600">
