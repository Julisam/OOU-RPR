...
class CustomUserAdmin(UserAdmin):
    list_display = ('username', 'full_name', 'officialemail', 'department', 'academic_rank', 'is_staff')
    list_filter = ('academic_rank', 'department', 'is_staff', 'is_active')
    search_fields = ('username', 'sname', 'fname', 'email')
    
    fieldsets = (
        (None, {'fields': ('role', 'password')}),
        ('Personal info', {'fields': ('sname', 'fname', 'mname', 'title', 'email', 'officialemail', 'phone_number')}),
        ('Academic info', {'fields': ('department', 'academic_rank', 'specialization', 'orcid_id', 'google_scholar_id')}),
    )
    add_fieldsets = (
        (None, {
            'classes': ('wide',),
...
class ResearchAwardAdmin(admin.ModelAdmin):
    list_display = ('user', 'award_name', 'awarding_organization', 'country', 'year_received')
    list_filter = ('country', 'year_received')
    search_fields = ('award_name', 'user__username')


@admin.register(JournalIndexStatus)
class JournalIndexStatusAdmin(admin.ModelAdmin):
    list_display = ('name',)
    search_fields = ('name',)
...
class EditorialAppointmentAdmin(admin.ModelAdmin):
    ...
    filter_horizontal = ('indexing_status',)
